...
def check_login(username, password):
    ...
    try:
        ...
        sql = """SELECT *
                 FROM mediaserver.useraccount u
...
def is_superuser(username):
    ...
    try:
        ...
        sql = """SELECT isSuper
                 FROM mediaserver.useraccount u
                 WHERE u.username = %s AND isSuper"""
...
def user_playlists(username):
    ...
    try:
        ...
        sql = """
            ...
            from mediaserver.mediacollection m
            where m.username = %s
        """
...
def user_podcast_subscriptions(username):
    ...
    try:
        ...
        sql = """
        SELECT *
        FROM mediaserver.Subscribed_Podcasts sp
        WHERE sp.username = %s
        """
...
def user_in_progress_items(username):
    ...
    try:
        ...
        sql = """
         SELECT *
         FROM MediaServer.UserMediaConsumption u
...
def get_song(song_id):
    ...
    try:
        ...
        #############################################################################
        sql = """
            SELECT s.song_title as "Song Name", a.artist_name as "Song Artist",s.length as "Song Length"
...
def get_song_metadata(song_id):
    ...
    try:
        ...
        sql = """
            SELECT md.md_value as Metadata
            FROM mediaserver.metaData md INNER JOIN mediaserver.MediaItemMetadata mimd USING (md_id)
            INNER JOIN mediaserver.mediaItem mi USING (media_id)
            WHERE mi.media_id = %s
        """
...
def get_podcast(podcast_id):
    ...
    try:
        ...
        #############################################################################
        sql = """
        select podcast_id as "podcast ID",podcast_title as "podcast title",podcast_uri as "url",podcast_last_updated as "Last Update",md_type_name as "Type"
...
def get_all_podcasteps_for_podcast(podcast_id):
    ...
    try:
        ...
        sql = """
        select podcast_id,media_id,podcast_episode_title,podcast_episode_published_date,podcast_episode_length
...
def get_podcastep(podcastep_id):
    ...
    try:
        ...
        #############################################################################
        sql = """
        select podcast_id,media_id,podcast_episode_title,podcast_episode_uri,podcast_episode_published_date,podcast_episode_length,md_type_name
...
def get_album(album_id):
    ...
    try:
        ...
        #############################################################################
        sql = """
            select album_title as Album_Name, md_value as Artwork
...
def get_album_songs(album_id):
    ...
    try:
        ...
        #############################################################################
        sql = """
        select song_id as Song_Number, album_id as Album_Number, song_title as Songs, artist_name as Artists
...
def get_album_genres(album_id):
    ...
    try:
        ...
        #############################################################################
        sql = """
            select DISTINCT( md_value ) as SONG_GENRE, song_title as Song_Name
            from MediaServer.album natural join MediaServer.album_songs natural join MediaServer.song natural join MediaServer.mediaitemmetadata natural join MediaServer.metadata natural join MediaServer.metadatatype
...
def get_tvshow(tvshow_id):
    ...
    try:
        ...
        # including all relevant metadata       #
        #############################################################################
        sql = """select tvshow_title as TV_Show, md_value
		from mediaserver.tvshow natural join mediaserver.tvshowmetadata natural join mediaserver.metadata
		WHERE md_id = %s;
        """
...
def get_all_tvshoweps_for_tvshow(tvshow_id):
    ...
    try:
        ...
        #############################################################################
        sql = """SELECT tvshow_id as "ID", tvshow_episode_title, season, episode, air_date
...
def find_matchingmovies(searchterm):
    ...
    try:
        ...
        #############################################################################
        sql = """
            SELECT mv.movie_id, mv.release_year, mv.movie_title

            FROM  mediaserver.Movie mv
            WHERE mv.movie_title LIKE '%%s%'
            ORDER BY mv.movie_title asc
        """
