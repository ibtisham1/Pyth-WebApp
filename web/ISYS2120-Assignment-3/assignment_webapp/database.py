...
def check_login(username, password):
    ...
    try:
        ...
        sql = """
        SELECT * FROM mediaserver.useraccount u
...
def user_playlists(username):
    ...
    try:
        ...
        ###############################################################################
        sql = """
            select m.collection_id, count(mm.media_id), m.collection_name
            from mediaserver.mediacollection m join mediaserver.mediacollectioncontents mm using(collection_id)
            where m.username = %s
...
def user_podcast_subscriptions(username):
    ...
    try:
        ...
        FROM mediaserver.Subscribed_Podcasts sp inner join mediaserver.podcast p using (podcast_id)
        WHERE sp.username = %s
...
def user_in_progress_items(username):
    ...
    try:
        ...
        sql = """
         SELECT u.media_id,  u.play_count as Playcount,u.progress, u.lastviewed,  mi.storage_location
         FROM MediaServer.UserMediaConsumption u inner join mediaserver.mediaItem mi using (media_id)
         WHERE u.username = %s AND u.progress > 0 AND u.progress < 10
         ORDER BY u.media_id
        """
...
def get_alltvshows():
    ...
    try:
        ...
        #############################################################################
        sql = """
        SELECT show.tvshow_id, show.tvshow_title,COUNT(epi.media_id)
		FROM mediaserver.TVShow show, mediaserver.TVEpisode epi
		WHERE show.tvshow_id = epi.tvshow_id
		GROUP BY show.tvshow_id
        order by show.tvshow_id
        """
...
def get_song(song_id):
    ...
    try:
        ...
        #############################################################################
        sql = """
        SELECT s.song_title, a.artist_name as Artists,s.length
        FROM mediaserver.song s INNER JOIN mediaserver.Song_Artists sr USING (song_id) INNER JOIN mediaserver.artist a
        ON sr.performing_artist_id = a.artist_id
        WHERE s.song_id = %s
        """
...
def get_podcastep(podcastep_id):
    ...
    try:
        ...
        sql = """
            select *
            from mediaserver.podcastepisode join MediaServer.podcastmetadata using (podcast_id) join MediaServer.metadata using (md_id) join MediaServer.metadatatype using (md_type_id)
            where media_id = %s
        """
...
def get_album(album_id):
    ...
    try:
        ...
        sql = """
        select *
        from MediaServer.albummetadata mm inner join MediaServer.metadata using (md_id)
...
def get_album_songs(album_id):
    ...
    try:
        ...
        #############################################################################
        sql = """
        select DISTINCT(song_id) , album_id, song_title, artist_name as artists

        from MediaServer.album_songs inner join mediaserver.song using (song_id)
        inner join mediaserver.song_artists sa using(song_Id)
        inner join MediaServer.artist a on (sa.performing_artist_id = artist_id)
...
def get_album_genres(album_id):
    ...
    try:
        ...
        from MediaServer.album_songs a inner join MediaServer.mediaitemmetadata mi on
        (a.song_id = mi.media_id) join MediaServer.metadata using(md_id)
        inner join MediaServer.metadatatype using(md_type_id)
        where album_id = %s and MediaServer.metadatatype.md_type_name = 'song genre'
        """
...
def get_tvshow(tvshow_id):
    ...
    try:
        ...
        sql = """
        select *
        from mediaserver.TVshow m natural join
...
def find_matchingmovies(searchterm):
    ...
    try:
        ...
        from
            mediaserver.movie m,
            (select
                m1.movie_id, count(m1.movie_id) as count
            from
                mediaserver.movie m1
                group by m1.movie_id) mnew
...
#####################################################
#   extra feature supporting queries
#
#####################################################
def get_contact(username):
    """
    Get the corresponding user contact deatils
    """
    print('contact is being executed')
    conn = database_connect()
    if(conn is None):
        return None
    cur = conn.cursor()
    try:
        sql = """
        select *
        from mediaserver.contactmethod c inner join mediaserver.contacttype using (contact_type_id)
        where c.username=%s;
        """
        ...
        # If there were any errors, return a NULL row printing an error to the debug
        print("Unexpected error getting user contact:", sys.exc_info()[0])
        raise
    cur.close()                     # Close the cursor
    conn.close()                    # Close the connection to the db
    return None
...
def update_password(username,password):
    ...
    print('update password is called')
    conn = database_connect()
    if(conn is None):
        return None
    cur = conn.cursor()
...
def change_email(username,email):
    ...
    try:
        ...
        # If there were any errors, return a NULL row printing an error to the debug
        print("Unexpected error changing email method:", sys.exc_info()[0])
        raise
    conn.commit()
    cur.close()                     # Close the cursor
    conn.close()                    # Close the connection to the db
...
def change_phoneNo(usern,phone):
    """
    update email of a user
    """

    print('update phone is called')
    conn = database_connect()
    if(conn is None):
        return None
    cur = conn.cursor()
    try:
        ...
        sql = """
        SELECT
            mediaserver.changePhoneNo(
                %s,%s);
        """

        cur.execute(sql,(usern,phone))
        r = cur.fetchone()
        print("return val is:")
        print(r)
        conn.commit()
        ...
        # If there were any errors, return a NULL row printing an error to the debug
        print("Unexpected error changing Phone Number:", sys.exc_info()[0])
        raise
    conn.commit()
    cur.close()                     # Close the cursor
    conn.close()                    # Close the connection to the db
...
def change_social(usern,social):
    """
    update social of a user
    """

    print('update social is called')
    conn = database_connect()
    if(conn is None):
        return None
    cur = conn.cursor()

    outersql = """SELECT * FROM mediaserver.contactmethod c WHERE c.username = %s and c.contact_type_id = 3"""
    cur.execute(outersql,(usern,))
    v = cur.fetchone()
    print(v)
    ...
    try:
        ...
        if v == None:
            ...
            print("this is being executed")
            print("this is being executed")
            sql = """INSERT INTO mediaserver.contactmethod(username,contact_type_id,contact_type_value)
            VALUES (%s,3,%s)
            """
        else:
            sql = """
            SELECT
                mediaserver.changeSocial(
                    %s,%s);
            """
        cur.execute(sql,(usern,social,))
        #r = cur.fetchone()
        #print("return val is:")
        #print(r)
        conn.commit()
