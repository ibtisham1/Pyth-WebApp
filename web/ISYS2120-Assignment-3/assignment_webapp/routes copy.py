...
def single_podcastep(media_id):
    ...
    page['title'] = 'podcast_episode' # Add the title

    # Set up some variables to manage the returns from the database fucntions
    podcastep1 = None
    podcastep1 = database.get_podcastep(media_id)
...
def search_movies():
    ...
    if request.method == 'POST':
        ...
        # Once retrieved, do some datta integrity checks on the data
...
def add_song():
    ...
    if(request.method == 'POST'):
        ...
        if ('performing_artist_id' not in request.form):
            newdict['performing_artist_id'] = '0'
        else:
            newdict['performing_artist_id'] = request.form['performing_artist_id']
            print("We have a value: ",newdict['performing_artist_id'])
        ...
        #forward to the database to manage insert
        songs = database.add_movie_to_db(newdict['song_title'],newdict['performing_artist_id'],newdict['description'],newdict['storage_location'],newdict['song_genre'])
...
def display_contact(username):
    """
    get corresponding contact details for a specific user
    """
    # # Check if the user is logged in, if not: back to login.
    if('logged_in' not in session or not session['logged_in']):
        return redirect(url_for('login'))

    page['title'] = 'Contact Deatils of: '+ username

    # Get a list of contact of this user
    contact = None
    contact = database.get_contact(username)
...
@app.route('/cpassword/<username>',methods=['POST','GET'])
def change_password(username):
    """
    allow user to update personal password
    """
    # # Check if the user is logged in, if not: back to login.
    if('logged_in' not in session or not session['logged_in']):
        return redirect(url_for('login'))

    page['title'] = 'Change '+ username + """'s password."""
    ...
    if(request.method == 'POST'):
        ...
        if ('password' not in request.form):
            newdict['password'] = 'Empty Password'
        else:
        ...
        #forward to the database to manage insert
        u = database.update_password(newdict['username'],newdict['password'])

        print(u)

        # change password successfully, now redirect the user back to contact page
        contact = None
        contact = database.get_contact(username)
...
@app.route('/changeEmail/<username>',methods=['POST','GET'])
def changeEmail(username):
    """
    allow user to change email information
    """
    # # Check if the user is logged in, if not: back to login.
    if('logged_in' not in session or not session['logged_in']):
        return redirect(url_for('login'))

    page['title'] = 'Change'+ username + """'s Email."""
    ...
    if(request.method == 'POST'):
        ...
        if ('email' not in request.form):
            newdict['email'] = 'Empty Email'
        else:
            newdict['email'] = request.form['email']
            print("We have a value: ",newdict['email'])
        ...
        #forward to the database to manage insert
        u = database.change_email(newdict['username'],newdict['email'])
...
@app.route('/changePhoneNo/<username>',methods=['POST','GET'])
def changePhoneNo(username):
    """
    ...
    page['title'] = 'Change'+ username + """'s Phone."""
    ...
    if(request.method == 'POST'):
        ...
        if ('phone' not in request.form):
            newdict['phone'] = 'Empty Phone'
        else:
            newdict['phone'] = request.form['phone']
            print("We have a value: ",newdict['phone'])
        ...
        #forward to the database to manage insert
        u = database.change_phoneNo(newdict['username'],newdict['phone'])
        ...
        return render_template('changephoneno.html',
                           session=session,
                           page=page,
                           user=user_details)


@app.route('/changeSocial/<username>',methods=['POST','GET'])
def changeSocial(username):
    """
    allow user to change Social
    """
    # # Check if the user is logged in, if not: back to login.
    if('logged_in' not in session or not session['logged_in']):
        return redirect(url_for('login'))

    page['title'] = 'Change'+ username + """'s Social Details."""
    ...
    if(request.method == 'POST'):
        ...
        # verify that the values are available:
        if ('social' not in request.form):
            newdict['social'] = 'Empty Social Information'
        else:
            newdict['social'] = request.form['social']
            print("We have a value: ",newdict['social'])
        ...
        #forward to the database to manage insert
        u = database.change_social(newdict['username'],newdict['social'])
        ...
        return render_template('changesocial.html',
                           session=session,
                           page=page,
                           user=user_details)
