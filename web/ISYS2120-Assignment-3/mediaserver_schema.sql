...
-- Call it using 'Select updatePass([paramaters]);'
-- Example: SELECT mediaserver.updatePass('james.smith','newpass');
create or replace function mediaserver.updatePass(
	usern varchar(50),
  pass varchar(100)
  )
...
RETURNS text AS
$BODY$
      UPDATE mediaserver.useraccount u
      SET password = pass
...
-- Function to update contactInfo of a user
-- Call it using 'Select updateInfo([paramaters]);'
-- Example: SELECT mediaserver.updateInfo('james.smith','newcontactmethod');
...
RETURNS text AS
$BODY$
      UPDATE mediaserver.contactmethod u
      SET contact_type_value = email
      WHERE username = usern and u.contact_type_name = 1
	  RETURNING contact_type_value;
$BODY$
LANGUAGE sql;

create or replace function mediaserver.changePhoneNo(
	usern varchar(50),
  phone varchar(20)
  )
...
$BODY$
      ...
      WHERE username = usern and contact_type_name = 2
	  RETURNING contact_type_value;
$BODY$
LANGUAGE sql;
...
create or replace function mediaserver.changeSocial(
	usern varchar(50),
  social varchar(100)
  )
...
$BODY$
        ...
        WHERE username = usern and contact_type_id = 3
	  RETURNING contact_type_value;
$BODY$
LANGUAGE sql;
